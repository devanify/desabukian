    ...
    <div class="max-w-[85rem] px-4 py-10 sm:py-5 sm:px-6 lg:px-8  mx-auto mb-[100px]">
      <div class="max-w-2xl mx-auto text-center mb-10 lg:mb-14">
        ...
        <div class="hs-dropdown relative inline-flex mt-3">
          ...
          <div class="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-60 bg-white shadow-md rounded-lg mt-2 divide-y divide-gray-200" role="menu" aria-orientation="vertical" aria-labelledby="hs-dropdown-with-icons">
            <div class="p-1 space-y-0.5">
              @foreach ($years as $year)
              <a href="{{ route('gallery', ['year' => $year]) }}" class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:outline-none focus:bg-gray-100">
                {{ $year }}
              </a>
              @endforeach
            </div>
        ...
        @foreach ($galeri as $monthYear => $images)
                ...
                @foreach ($images as $g)
                    <div>
                        <img class="h-auto max-w-full rounded-lg"
                            src="{{ asset('assets/image/galeri/' . $g->image_url) }}" alt="Gallery image" />
